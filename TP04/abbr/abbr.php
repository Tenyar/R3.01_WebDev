...
class Abbr {
  ...
  function show(string $abbr) : string {
      if(array_key_exists($abbr, $this->abbr)){ // si la clé existe dans la liste de l'objet 'Abbr'.
        $value = $this->abbr[$abbr];
        $key = $abbr;
    
        $abbr ="<abbr title=\"$value\">$key</abbr>";
        return $abbr;
      }
      else{
        return $abbr;
  ...
  $keywords = preg_split("/[\s,]+/", $text);
    ...
    {
      // si le string == clé dans la liste
